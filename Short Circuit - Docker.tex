...
A Dockerfile in a repository is a plain-text document that tells Docker how to build, that is, \emph{containerize} the app and dependencies into an image, which you can do with the \mintinline{bash}{docker build -t <image>:<version>} command.
...
\emph{Microservices applications} are modern cloud-native applications that consist of lots of small services that work together, called \emph{microservices}. Instead of hacking together complex scripts and long docker commands, Docker Compose lets you describe the application in a \mintinline{bash}{compose.yaml} file, which you use with the \mintinline{bash}{docker compose} command to deploy and manage the app. Docker Compose started as a Python too called Fig by a company called Orchard Labs. Docker, Inc. acquired Orchard Labs and rebranded Fig as Docker
...
You can deploy this with \mintinline{bash}{docker compose up &}, shut it down with \mintinline{bash}{docker compose down}, stop it with \mintinline{bash}{docker compose stop} (this won't deleted any of its resources or containers), restart it with \mintinline{bash}{docker compose restart}\footnote{If you make changes to the Compose file while it’s stopped, these changes will not appear in the restarted app.}, list its containers with \mintinline{bash}{docker compose ls}, and list the processes inside each container with \mintinline{bash}{docker compose top}.\footnote{The PID numbers returned are the PID numbers as seen by the Docker host (not from within the containers).}. If your Compose file is not called \mintinline{bash}{compose.yml} you can specify if with the \mintinline{bash}{-f} flag. You can use the \mintinline{bash}{--detach} flag to deploy the app in the background. Since Compose is working with regular Docker constructs behind the scenes, you can use regular \mintinline{bash}{docker} commands to check the deployment. Created (i.e. not pulled) images will be called \mintinline{bash}{<build_context_dir_name>-<service_name>}, containers (from both created and pulled images) will be called \mintinline{bash}{<build_context_dir_name>-<service_name>-<replica_num>}. Networks and volumes are named similarly. You can list networks and volumes with \mintinline{bash}{docker network ls} and \mintinline{bash}{docker volume ls}, respectively.\\
Docker decouples the lifecycle of volumes from the rest of the application, so \mintinline{bash}{docker compose down} removed containers and networks, but not volumes, because there might be important information stored on there. You can also remove the volumes with the \mintinline{bash}{--volumes} flag to \mintinline{bash}{docker compose down}. Images are also not automatically deleted, such that subsequent deployments are faster; use the \mintinline{bash}{--rmi all} flag to delete the images.

...
At runtime, the Wasm app and Wasm runtime will execute inside a minimal container that is basically just namespaces and cgroups (no filesystem etc.). Use \lstinline{docker build --platform wasi/wasm --provenance=false -t <target_image>} to containerize the Wasm app into an OCI image. Run it with \lstinline{docker run -d --name wasm-ctr --runtime=io.containerd.spin.v2 --platform=wasi/wasm -p 5556:80 nigelpoulton/ddd-book:wasm /}. The\lstinline{--runtime} flag makes sure Docker executes the container with the spin Wasm runtime.
...
Docker Hub maintains a catalog of verified models below the \href{https://hub.docker.com/u/ai}{ai namespace}. \emph{Models} are a new (proposed) OCI artifact type. The Docker CLI can pull and push models from Docker Hub and DMR stores them in a \emph{local model store} (in your filesystem below \lstinline{~/.docker/models}) for fast access. They're typically several gigabytes in size. You can use \lstinline{docker model pull <model>} to pull models and \lstinline{docker model ls} to list local models. These commands may be merged into the standard \lstinline{pull} and \lstinline{ls} later on.

DMR is a host process separate from the Docker Engine. It wraps one or more pluggable \emph{runtimes} aka \emph{core inference engines} (such as \lstinline{llama.cpp}), which in turn use acceleration hardware APIs to load and execute models and provide inference. DMR exposes API endpoints for model management (i.e. querying and managing models) and inference (these endpoints are OpenAI-compatible). Containers on the same host can access the endpoints via the \lstinline{http://model-runner.docker.internal/} hostname, non-containerized apps on the same host can reach them via the local Docker socket or the host’s loopback address on port 12434, and apps running on different hosts can access them via the DMR host’s IP or DNS name on port 12434.
